...
# def _build_llm_client():
#     """Returns an OpenRouterLLMClient if OPENROUTER_API_KEY is set, else
#     None. None is a fully supported state — the narrative endpoint falls
#     back to the deterministic template and reports
#     grounding_status='fallback_no_llm_configured', it never errors.

#     Model defaults to a free-tier OpenRouter model (liquid/lfm-2.5-2.6b:free).
#     Override with OPENROUTER_MODEL if you want a different one."""
#     api_key = os.environ.get("OPENROUTER_API_KEY")
#     if not api_key:
#         logger.warning("OPENROUTER_API_KEY not set — narrative endpoint will use the deterministic fallback template only.")
#         return None
#     from app.narrative.llm_client import OpenRouterLLMClient
...
def create_app(db_path: str = "swasthiq.db") -> FastAPI:
    ...
    async def unhandled_exception_handler(request: Request, exc: Exception):
        ...
        # bad billing DATA is a normal response (422/201+errors), not an
        # exception. This handler is for genuine bugs/infra failures, and
        # deliberately does NOT leak the stack trace to the client BY
        # DEFAULT — set SWASTHIQ_DEBUG=1 to include it in the response
        # body during local debugging. Never set this in production.
        logger.exception("Unhandled exception on %s %s", request.method, request.url)
        content = {
            "error": "internal_server_error",
            "detail": "An unexpected error occurred and has been logged.",
        }
        if os.environ.get("SWASTHIQ_DEBUG") == "1":
            import traceback
            content["debug_traceback"] = traceback.format_exc()
            content["debug_exception_type"] = type(exc).__name__
            content["debug_exception_message"] = str(exc)
